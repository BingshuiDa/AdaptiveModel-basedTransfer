function [bestSol, bestfit_hist] = CBR_DPB(h_len)
load allmodels_CBR

pop = 100;
generation = 100;
TrInt = 10;
injection = round(0.1*pop);
...
% population = 12*rand(pop-size(allmodels_CBR,1), nVariable) - 6;
% population = [population; allmodels_CBR];
...
[bestfit_hist(1), ind] = max(fitness);
...
for gen = 2:generation
    ...
    if mod(gen-2, TrInt) == 0
        dist = sq_dist(allmodels_CBR', bestSol');
        [~, ind] = sort(dist);
        child((pop-injection)+1:end,:) = allmodels_CBR(ind(1:injection),:); 
    end
