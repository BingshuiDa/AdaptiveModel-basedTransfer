  ...
  methods (Static)
    ...
    function model = buildmodel(model,solutions)
      ...
      if strcmp(model.modeltype,'mvarnorm')
        ...
        stderr = std(solutions);
        model.covarmat_true = diag(stderr.^2); % Simplifying to univariate distribution by ignoring off diagonal terms of covariance matrix
        solutions_noisy = [solutions;rand(round(0.2*pop),model.vars)];
        model.mean_noisy = mean(solutions_noisy);
        stderr = std(solutions_noisy);
        model.covarmat_noisy = diag(stderr.^2);% Simplifying to univariate distribution by ignoring off diagonal terms of covariance matrix
%         model.covarmat_noisy = cov(solutions_noisy);
      elseif strcmp(model.modeltype,'umd')
        model.probofone_true = mean(solutions);
        model.probofzero_true = 1 - model.probofone_true;
        solutions_noisy = [solutions;round(rand(0*pop,model.vars))];
